...
public class Game {
    ...
    public String checkGameWinner(char[][] grid) {
        ...
        boolean win = false;
        char[] symbols = {'x', 'o'};
        String[] message = {"x wins", "o wins", "tie", result};

        for (int symb = 0; symb < symbols.length; symb++) {
            // check diagonals
            if ( grid[0][0] == symbols[symb] && grid[1][1] == symbols[symb] && grid[2][2] == symbols[symb]
                    || grid[0][2] == symbols[symb] && grid[1][1] == symbols[symb] && grid[2][0] == symbols[symb] ){
                return  message[symb];
            }
            for(int i=0; i<grid.length; i++){
                //check who wins in columns
                if( grid[i][0] == symbols[symb] && grid[i][1] == symbols[symb] && grid[i][2] == symbols[symb]){
                    return message[symb];
                }
                //check who wins in rows
                if(grid[0][i] == symbols[symb] && grid[1][i] == symbols[symb] && grid[2][i] == symbols[symb] ){
                    return message[symb];
                }
            }
            if(freeSpots == 0 ){
